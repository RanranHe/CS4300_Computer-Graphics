...
public abstract class AbstractNode implements INode
{
    ...
    public void descendLights(List<Light> result, Matrix4f modelView) {
        throw new UnsupportedOperationException("don't know how to descend an abstract");
    }

    @Override
    public Matrix4f objectToView(Matrix4f acc, Matrix4f modelView) {

        throw new UnsupportedOperationException("don't need to trans a abstract to view");
    }

    @Override
    public void transformedToLight(Matrix4f objectToView, List<Light> transformedLights) {
       for (Light light : transformedLights) {

           if (light.getPosition().w==1) {
               light.setPosition(new Vector4f(light.getPosition())
                 .mul(objectToView));

               Vector4f newMatrix = new Vector4f(light.getSpotDirection().mul(objectToView));
               light.setSpotDirection(newMatrix.x, newMatrix.y, newMatrix.z);
           }
           else {
               light.setDirection(new Vector4f(light.getPosition().mul(objectToView)));
           }
       }
